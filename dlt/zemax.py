...
def zemax_spherical_to_jax_array(state, focal_length=None, object_distance=jnp.inf):
    curvs = jnp.pad(state[:, 0], (0,1), constant_values=0.0)
    positions = jnp.pad(jnp.cumsum(state[:, 1]), (1,0), constant_values=0.0)
    iors = jnp.pad(state[:, 2], (1,1), constant_values=constants.DEFAULT_EXT_IOR)
    semidiams = jnp.pad(state[:, 3], (0,1), constant_values=constants.FLOAT_BIG_NUMBER)

    sensor_pos = positions[-1]
    if focal_length is not None:
        elp = sensor_pos - focal_length
        u = jnp.abs(object_distance + elp)
        v = 1 / ((1 / focal_length) - (1 / u))
        sensor_pos = elp + v
    positions = positions.at[-1].set(sensor_pos)

    surfs = jax.vmap(curvature_sphere.curvature_sphere)(curvs, positions, semidiams, iors[:-1], iors[1:])
    return surfs
