...
def load_zemax_generic(fname, material_override=False):
    ...
    surface_descriptions = [dict()] * len(surfaces)
...
def desc_state_to_array(desc_state, wavelength=587.56):
    lens = np.zeros((len(desc_state)-2, 13))
    for i, s in enumerate(desc_state[1:-1]):

        lens[i, 0] = s['curvature']
        lens[i, 1] = s['distance'] if isinstance(s['distance'], float) else s['distance'][0]
        lens[i, 2] = s['ior_fn'](wavelength)
        lens[i, 3] = s['semidiam'] if isinstance(s['semidiam'], float) else s['semidiam'][0]
...
def sanitize_zemax_lens(lens, lens_type_list) -> Tuple[Float, List[str]]:
    '''Remove the stop and any blank surfaces from the beginning of the lens
        Useful for when we want to optimize the lenses without thinking about the aperture stop
    '''
    # if ZemaxSurfaceType.STOP in lens_type_list:
    #     stop_idx = lens_type_list.index(ZemaxSurfaceType.STOP)
    #     lens_type_list = lens_type_list[:stop_idx] + lens_type_list[stop_idx+1:]
    #     extra_dist = lens[stop_idx, 1]
    #     lens = np.concatenate([lens[:stop_idx], lens[stop_idx+1:]], axis=0)
    #     lens[stop_idx-1, 1] += extra_dist

    if lens[0, 2] <= lens[-1, 2]:
        # the lens has a non surface at the beginning for some reason?
        lens = lens[1:, :]
        lens_type_list = lens_type_list[1:]

    # remove any surfaces that have the same IOR as the next one
    positions = (lens[:, 1].cumsum() - lens[:, 1]).tolist()
    curvs = lens[:, 0].tolist()
    iors = lens[:, 2].tolist()
    semidiams = lens[:, 3].tolist()
    mask = [False] + [iors[i-1] == iors[i] for i in range(1, len(iors))]

    lens_list = [
        np.array([c, p, i, s]) 
        for c, p, i, s, m in zip(curvs, positions, iors, semidiams, mask) 
        if not m
    ]
    nlens = np.array(lens_list)
    nlens[:-1, 1] = np.diff(nlens[:, 1])
    nlens[-1, 1] = lens[:, 1].sum() - nlens[-1, 1]

    lens_type_list = [l for l, m in zip(lens_type_list, mask) if not m]
    return nlens, lens_type_list
...
def load_zemax_file(fname, non_standard=False, info_list=False):
    ...
    for i, s in enumerate(surfaces[1:-1]):
        ...
        state[i, 2] = surf_ior
        state[i, 3] = fields.get("DIAM", constants.FLOAT_BIG_NUMBER)
...
def load_glass_catalogs(*catalog_names):
    ...
    for cn in catalog_names:
        fname = os.path.join("data/glass_catalogs", cn.upper() + ".AGF")
...
def load_glass_ior_function(glass_name, info=None):
        ...
        elif glass_name.upper() in ['VACUUM', 'AIR']:
            dispform = 0
            cd = [0.0]*6
        else:
            raise ValueError('must include info for non-air/vacuum glasses')
        ...
        elif (glass_name.upper() == 'AIR') or (dispform == 0):
