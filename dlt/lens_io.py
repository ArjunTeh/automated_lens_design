...
def clamp_semidiameter(lens):
    ...
    k2 = lens[1:, 0]
    th = lens[:-1, 1]
    max_height_k1, max_height_k2 = optical_properties.effective_semidiameter(k1, k2, th)

    lens_clamp = np.array(lens)
    lens_clamp[:-1, 3] = np.minimum(lens[:-1, 3], max_height_k1)
    lens_clamp[1:, 3] = np.minimum(lens[1:, 3], max_height_k2)
    return lens_clamp
...
def export_lens(fname, lens, lens_desc_generic=None, wavelengths=[587.56], distances=None, surf_ids=None):
    ...
    if lens_desc_generic is None:
        ...
        # eflmag = optical_properties.effective_focal_length_magnification_from_state(lens)
