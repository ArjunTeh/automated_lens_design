...
def bisection_search(fn, low, hi, max_iter=12, tol=1e-8):
    ...
    def body_fun(carry):
        ...
        aux = jax.lax.cond(mid_pass, lambda : mid_aux, lambda : aux)
...
def calc_zemax_na_angle(state, angle, stop_id):
    ...
    def check_ray(val):
        ...
        (valid, xp, vp, Lp), (Tx, wargmax) = tracing.trace_scan_no_stop(surf_funs, surfs, x, v, L, stop_id)
        return valid, (wargmax, xp)

    middle = 0.0001
    midpt = jnp.sin(angle) * middle
    max_val = state[0, 3]
    hi_bound, hi_aux = bisection_search(check_ray, midpt, max_val)
    lo_bound, lo_aux = bisection_search(check_ray, midpt, -max_val)
    return (lo_bound, lo_aux), (hi_bound, hi_aux)
