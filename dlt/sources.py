...
def point_source_quadrature(half_length, angle, distance):
    ...
    def generate_rays(nrays, key=None):
        ...
        return x, v, rad, weights, rad

    return generate_rays


def line_source_quadrature(half_length, angle):
    def generate_rays(nrays, width, height, key=None):
...
def square_source_quadrature(width, height=None, angle=None, fpp=0.0):
    if height is None:
        height = width
    ...
    def generate_rays(nrays, key):
        ...
        chief_pos = jnp.array([-1, jnp.tan(angle)*(fpp+1), 0.0])
        pos = jnp.stack([z, x, y], axis=1)
        vel = jnp.tile(jnp.stack([jnp.cos(angle), -jnp.sin(angle), 0.0]), (pos.shape[0], 1))
        rad2 = jnp.sum(jnp.square(pos), axis=-1)
        pos = pos + chief_pos
        weights = leggaus_weight_appx(rad2, jnp.maximum(width, height))
        xradius = jnp.ones_like(pos[:, 0])

        return pos, vel, jnp.sqrt(rad2), weights, xradius

    return generate_rays


def square_source_random_stratified(width, height=None, angle=None, fpp=0.0):
    if height is None:
        height = width
    ...
    def generate_rays(nrays, width, height, key):
        dx = jax.random.uniform(key, (nrays*nrays,), minval=-0.5, maxval=0.5) * width / nrays
        dy = jax.random.uniform(key, (nrays*nrays,), minval=-0.5, maxval=0.5) * height / nrays
        width_range = ((jnp.arange(nrays) + 0.5) / nrays - 0.5) * width
        height_range = ((jnp.arange(nrays) + 0.5) / nrays - 0.5) * height
        Xm, Ym = jnp.meshgrid(width_range, 
                              height_range)
        x, y = Xm.flatten() + dx, Ym.flatten() + dy
        z = -jnp.zeros_like(x)

        chief_pos = jnp.array([-1, jnp.tan(angle)*(fpp+1), 0.0])
        pos = jnp.stack([z, x, y], axis=1)
        vel = jnp.tile(jnp.stack([jnp.cos(angle), -jnp.sin(angle), 0.0]), (pos.shape[0], 1))
        L = jnp.ones((pos.shape[0],))
        rad2 = jnp.sum(jnp.square(pos), axis=-1)
        pos = pos + chief_pos
        weights = leggaus_weight_appx(rad2, jnp.maximum(width / 2, height / 2))
        area_of_source = jnp.ones_like(L) * width * height

        return pos, vel, L, weights, area_of_source
...
def leggaus_weight_appx(rad2, max_radius):
    '''returns an approximate weight for the leggaus quadrature
    '''
    nr2 = rad2 / (max_radius*max_radius)
    nr2 = jnp.clip(nr2, 0.0, 1.0)
    xr = 2 * nr2 - 1
    w = jnp.pi * jnp.sqrt(1 - xr**2)
    return w


def leggaus_circular_quadrature(diameter, rings, angles, src_angle=0.0, fpp=0.0, object_distance=np.inf):
    '''infinite conjugate only? '''
    radius = diameter / 2
    lens_pos, weights = leggauss_rings(1.0, rings, angles)
    ...
    offset = np.array([0.0, -(fpp+1) * np.tan(src_angle), 0.0])
    pos = lens_pos + offset
    radiance = np.ones((pos.shape[0],))
    x_rad = np.sqrt(np.sum(np.square(pos), axis=-1))

    def generate_rays(nrays, width, height, key):
        # interpret the width as diameter
        radius = width / 2
        pos = lens_pos * radius + offset
        area_of_source = jnp.ones_like(radiance) * jnp.pi * radius**2
        return pos, vel, radiance, weights, area_of_source

    return generate_rays


def leggauss_rings(radius, nrings, nangles):
    x, w = np.polynomial.legendre.leggauss(nrings)
    
    rho = np.sqrt((1 + x) / 2) * radius
    weights = w / 2

    pos_list = []
    ang_list = np.linspace(0, 2*np.pi, nangles + 1)[:-1]
    for a in ang_list:
        pos_list.append(np.stack([np.zeros_like(rho), np.cos(a) * rho, np.sin(a) * rho], axis=1))

    pos = np.concatenate(pos_list, axis=0)
    weights = np.tile(weights, (nangles,))
    return pos, weights / nangles
