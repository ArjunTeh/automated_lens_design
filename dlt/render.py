...
def point_src_to_lens(rng_key, pixel_pos, surf_to_sample : QuantizedRect, spp, pixel_size):
    ...
    cos2theta = jnp.abs(vel_to_surf[:, 0])**2
    importance = cos2theta * surf_area * pixel_area / dist**2
...
def render_zemax_image(lens, src : QuantizedRect, sens : QuantizedRect, image_sampler, spp=100, rng_key=None):
    ...
    image = vrender_point(srcX, srcY, srcL, nrays, src.distance, src.cone_angle, surf_funs, surfs, rng_key, True, spot_error=False)

    return image, (srcX, srcY)


def render_zemax_image_farfield(lens, sens : QuantizedRect, image_sampler, rng_key, spp=100, nbatches=1, verbose=False, nowarp=False, stop_id=None, offset=None, color=False):
    # setup tracer for lens
    surfs = zemax.zemax_state_to_jax_surfaces(lens, scene_scale=1.0)
    surfs = jnp.array(surfs[-2::-1]) # ignore the sensor surface since we are tracing from there
    surf_funs = curvature_sphere.functionSuite()
    ...
    if color:
        image = jnp.zeros((sens.hres, sens.wres, 3))
    else:
    ...
    for positions in tqdm.tqdm(jnp.split(src_positions, nbatches, axis=0), disable=not verbose):
        ...
        with jax.profiler.TraceAnnotation('render_generate_rays'):
            rng_keys = jax.random.split(rng_key, positions.shape[0]+1)
            rng_key, subkeys = rng_keys[0], rng_keys[1:]
            x, v, L, importance = vsrc(subkeys, positions + offset)
        x = x.reshape((-1, 3))
        v = v.reshape((-1, 3))
        L = L.reshape((-1,))
...
def trace_rays_to_sensor(surf_funs, surfs, rays, stop_id=None):
    ...
    x, v, L = rays
    (valid, xp, vp, Lp), (Tx, wargmin) = vtracer(surf_funs, surfs, x, v, L, stop_id, len(surfs))
